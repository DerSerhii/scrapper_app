...
async def get_page_data(page_url: str, session: ClientSession, number: int, max_page: int):
    # """Returns a list of data dictionaries for the given page."""

    async with session.get(url=page_url, headers=settings.HEADERS) as response:
        response_text = await response.text()
        print(f'Parsing page #{number} of {max_page}', end=('.' * 5))
        print(f'Status: {response.status}', end=('.'*5))
